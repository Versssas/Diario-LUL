...
export default function DiarioLUL() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const r = await window.storage.get("lul-posts-v5");
        if (r?.value) { const p = JSON.parse(r.value); setPosts(p.length ? p : SAMPLE_POSTS); }
  ...
  async function saveAll(updated) {
    try { await window.storage.set("lul-posts-v5", JSON.stringify(updated), true); } catch {}
  }
